...
const cooldownDataFile = "data/pool/cooldowns.json"
...
func (p *AccountPool) SaveCooldowns() error {
	...
	dir := filepath.Dir(cooldownDataFile)
	...
	return os.WriteFile(cooldownDataFile, jsonData, 0600)
}
...
func (p *AccountPool) loadCooldowns() error {
	...
	data, err := os.ReadFile(cooldownDataFile)
	if err != nil {
	...
	if loaded > 0 {
		logger.Infof("[Pool] Loaded %d active cooldowns from %s", loaded, cooldownDataFile)
	}
	return nil
}
