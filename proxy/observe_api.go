...
// apiObserveKeys GET /admin/api/observe/keys
// 返回每个多 Key 的当前 RPM/RPD 已用 + 累计统计。
func (h *Handler) apiObserveKeys(w http.ResponseWriter, r *http.Request) {
	...
	type row struct {
		...
		RPM          int     `json:"rpm"`
		RPD          int     `json:"rpd"`
		UsedMinute   int     `json:"usedMinute"`
